...
extern uint8_t recv_buf_MP3[10];
...
void mp3_send_cmd (uint8_t cmd, uint16_t low_arg) {
	mp3_send_cmd(cmd, 0, low_arg);delay(100);
}

//
void mp3_send_cmd (uint8_t cmd) {
	mp3_send_cmd(cmd, 0, 0);delay(100);
}
...
void mp3_send_cmd (uint8_t cmd, uint8_t byte1, uint32_t byte234) {
	...
	mp3_send_cmd (cmd, high, low);delay(100);
}
...
uint8_t* mp3_recv_cmd (uint8_t wait) {
	...
	do {
	    ...
	    if (read) {
	    	received = recv_buf_MP3[3];

	    	if (received == 0x40) {
	    		// Error responce
	    		(* _debug_serial).print("Error responce with code ");
	    		(* _debug_serial).println(recv_buf_MP3[6]);
	    	}else if (wait != 0 && wait == received) {
	            result[0] = recv_buf_MP3[5];
	            result[1] = recv_buf_MP3[6];
	    	}
	    } else {
	    	delay(50);
	    }
